...
const Orders = ({ token }) => {
  ...
  const deleteOrder = async (orderId) => {
    try {
      const response = await axios.delete(`${backendUrl}/api/order/order/${orderId}`, { headers: { token } })
      if (response.data.success) {
      ...
      <div>
        {
          orders.map((order, index) => (
            <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={order._id}>
              ...
              {/* Delete button */}
              <button onClick={() => deleteOrder(order._id)} className="p-2 bg-red-500 text-white rounded hover:bg-red-700">Delete</button>
            </div>
          ))
        }
      </div>
